    ...
    return (
        <>
         <div className='fixed top-12 right-0 w-11/12 -z-10 translate-y-[-80%]'>
           ...
           <nav className={`flex items-center justify-between px-5 lg:px-8 xl:px-[8%] py-4 z-50 w-full fixed ${isScroll ? 'bg-white bg-opacity-50 backdrop-blur-lg shadow-sm' : ""}`}>
              <a href="#top">
                <Image src={assets.my_logo} alt="logo" className='w-28 cursor-pointer mr-14'/>
              </a>

              <ul className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? ' ' : "bg-white shadow-smn bg-opacity-50"}`}>
